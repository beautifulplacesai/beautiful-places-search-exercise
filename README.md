...
```bash
uv run jupyter notebook tryout.ipynb
```

Part 0 should show you six sunlit photos within a minute (photos download from
Geograph on first view and cache locally). Then climb the ladder — difficulty is
marked ⭐ to ⭐⭐⭐⭐, and skipping is allowed.

## What's in the box

- `tryout.ipynb` — the exercise (generated by `make_tryout.py`; edit that, not the notebook)
