...
≈5,000 photos of ≈4,700 verified, named places in London. Every photo has been
checked by a human, is attached to a named place, and carries a **scenic score
(0–10)** from our model — a CNN trained on tens of thousands of human beauty
ratings. This measured-beauty data is the company's core asset: no public model
or dataset provides it.
...
# ---------------------------------------------------------------- part 4
md("""\
## 4 · A language model that uses our tools
...
framework is **LangChain**, the same one our production pipeline uses.
...
- **Supervised classifiers (the standard until ~2022).** One model per
  ...
  this generation: a CNN trained on tens of thousands of human beauty ratings.
